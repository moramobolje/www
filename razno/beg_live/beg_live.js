...
function setActiveButton(type) {
    ...
    buttons.forEach(btn => {
        // 2. If the button's text or function matches the type, make it active
  ...
  try {
    ...
    flightBlocks.forEach(block => {
      ...
      const flight = {
        ...
        // This ensures any Arrival (International or Domestic) is categorized correctly
        type: (getVal('TIP') === 'IA' || getVal('TIP') === 'DA') ? 'ARRIVALS' : 'DEPARTURES',
      };
...
function switchType(type, el) {
  currentType = type;
  // Change '.btn-pill' to '.btn-toggle' to match your HTML classes
